class SktalkController < ApiController
  ...
  def receive
    assertion = authenticator.verify_token(params[:token], audience: 'sktalk/receive')
  ...
  def receive_and_save_to_outbox
    assertion = authenticator.verify_token(params[:token], audience: 'sktalk/receive_and_save_to_outbox')
    receive_result = receiver(assertion).receive(params[:message])
